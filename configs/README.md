...
as `../data/marked-accounts/<snapshot>`, since a relative snapshot path resolves
against the selection naming it and these two kinds of selection sit in sibling
directories. The cut then leaves out every game a listed account played and
records the recall that snapshot had reached, which is where a corpus prepared
without one applies the rejection.
