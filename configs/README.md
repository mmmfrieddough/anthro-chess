...
A pool selection may also name one of the `data/marked-accounts/` snapshots. The
cut then leaves out every game a listed account played and records the recall
that snapshot had reached, which is where a corpus prepared without one applies
the rejection.
