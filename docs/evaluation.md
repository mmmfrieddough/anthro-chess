...
process, from its first statement to the moment it builds its cost record, and
a sweep's total is the sum of its steps plus what the driver pays.
...
varies along, and the machine prefix of every artifact path, since the
artifact is the same work wherever it is rooted.
