...
preserve. Excluding runs before subsampling, so `fraction` and `maximum_games`
still hold two such runs to one game count, and a run records the snapshot it
rejected against beside the resolved selection counting what it removed.
