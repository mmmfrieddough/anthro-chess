...
count and its own run name. A trunk, and one cooldown branched off it:

```console
uv run anthro train --config configs/training/sample-smoke.toml \
  --set 'run_name="training-smoke-trunk"' \
  --set cooldown_fraction=0.25 --set steps=8

uv run anthro train --config configs/training/sample-smoke.toml \
  --set 'run_name="training-smoke-cooled"' \
  --set cooldown_fraction=0.25 --set steps=16 \
  --set "resume_from=\"$ANTHRO_CHESS_RUN_ROOT/training-smoke-trunk/checkpoints/step-00000008.pt\""
```

The schedule settings are part of the identity a resume has to match, so a
branch differs from its trunk in the horizon and in where it resumed from, and
in nothing else. Its own run name is what keeps the trunk intact: a resume
writes into the run directory its name selects, and one pointed back at the
trunk would drop the trunk's metrics past the resumed step and leave nothing to
branch again from.
