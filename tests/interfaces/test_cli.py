...
def test_eval_freeze_reports_the_pool_it_cut_by_axis(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """What a reading resolves on an axis follows from the games behind it."""
    ...
    command_output = capsys.readouterr().out
    assert "Per-axis coverage:" in command_output
...
def test_the_cross_conditioning_star_marks_the_band_not_the_winner() -> None:
    ...
    assert "away-band penalty 0.250000 over 400 position(s)" in rendered
...
def test_the_within_game_table_sizes_both_halves() -> None:
    """The median split puts ties in the weaker half, so the halves differ."""
    ...
    from anthro_chess.interfaces.cli import _render_within_game

    def group(name: str, count: int, alignment: float) -> WithinGameGroup:
        return WithinGameGroup(
            rating_band="under_1200",
            group=name,
            position_count=count,
            mean_prefix_strength=-0.5 if name == WEAKER_PREFIX_GROUP else 0.5,
