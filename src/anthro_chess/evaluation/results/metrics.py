...
BENCHMARK_COST_FAMILY = register_family(
    MetricFamily(
        ...
        summary=(
            ...
            "took it and under the configuration it was configured with. Read "
            "to decide what a sweep can afford rather than to judge a "
            "checkpoint."
        ),
    )
)
...
BENCHMARK_WALL_CLOCK_SECONDS = register_metric(
    MetricDefinition(
        ...
        summary=(
            ...
            "loading its inputs to computing its last measurement. Interpreter "
            "startup and the store append are outside it, so a sweep's total is "
            "the sum of its steps plus what the driver itself pays."
        ),
        cost=MetricCost.MEASURED_EXECUTION,
        execution_sensitive=True,
    )
